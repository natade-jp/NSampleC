...
# おそらく、「/usr/local/lib」などが書いてあると思う。つまりそこにおけばいいです。
# 置いた場合は、「ldconfig」を使用して、共有ライブラリの依存関係情報を更新する必要があります。
# なお、現在の情報は「ldconfig -p」で表示が可能です。
# 環境変数 LD_LIBRARY_PATH ライブラリが書いてあるパスを通す方法もあります。
# ただし、LD_LIBRARY_PATH は一時的なものになります。
